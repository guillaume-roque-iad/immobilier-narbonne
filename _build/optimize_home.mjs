...
if (!Array.isArray(catalogue.items) || catalogue.items.length < 9) {
  throw new Error('Le catalogue doit contenir au moins neuf annonces.');
}
...
const cards = catalogue.items.slice(0, 9).map(renderCard).join('');
html = `${html.slice(0, start)}${gridStart}${cards}${html.slice(end)}`;

await writeFile(indexPath, html);
console.log(`Accueil optimisé : 9 annonces pré-rendues sur ${catalogue.items.length}.`);
